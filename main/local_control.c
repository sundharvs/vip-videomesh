                                        ...
                                        int32_t event_id, void *event_data)
{
    ...
    sta_addr = event->ip_info.ip.addr;

    //ip_portmap_add(IP_PROTO_TCP, event->ip_info.ip.addr, 8554, ESP_IP4TOADDR(192, 168, 4, 2), 8554);
    //ip_portmap_add(IP_PROTO_UDP, event->ip_info.ip.addr, 8554, ESP_IP4TOADDR(192, 168, 4, 2), 8554);
    //if (g_sockfd != -1) {
    //    close(g_sockfd);
    //    g_sockfd = -1;
    //}
                                        ...
                                        int32_t event_id, void *event_data)
{
    ...
    ESP_LOGI(TAG, "STA Connected to SoftAP with IP Address:" IPSTR, IP2STR(&event->ip));

    ip_portmap_add(IP_PROTO_TCP, sta_addr, 8554, event->ip.addr, 8554); // RTSP TCP Port
    ip_portmap_add(IP_PROTO_UDP, sta_addr, 8554, event->ip.addr, 8554); // RTSP UDP Port
    ip_portmap_add(IP_PROTO_TCP, sta_addr, 22, event->ip.addr, 22); // SSH TCP Port
    ip_portmap_add(IP_PROTO_UDP, sta_addr, 22, event->ip.addr, 22); // SSH UDP Port
    ip_portmap_add(IP_PROTO_TCP, sta_addr, 14551, event->ip.addr, 14551); // QGroundControl TCP Port
    ip_portmap_add(IP_PROTO_UDP, sta_addr, 14551, event->ip.addr, 14551); // QGroundControl UDP Port
    ip_portmap_add(IP_PROTO_TCP, sta_addr, 11311, event->ip.addr, 11311); // ROS TCP Port
    ip_portmap_add(IP_PROTO_UDP, sta_addr, 11311, event->ip.addr, 11311); // ROS TCP Port
    ESP_LOGI(TAG, "Added portmap");
}
...
void app_main()
{
    ...
    ESP_ERROR_CHECK(esp_event_handler_instance_register(IP_EVENT, IP_EVENT_AP_STAIPASSIGNED, &ip_event_ap_staipassigned_handler, NULL, NULL));
